...
$$
X_i \sim N(0,1), \qquad 1\le i\le n_X
$$
and 
$$
Y_i \sim \left\{
...
\right.
$$
where $z$ is the size of the outlier. We will run a simulation
for a grid of $z$ values and record the proportion of $p$-values
below $0.05$ as $z$ increases.
...
zg <- seq(0,20,,100)
P <- matrix(0,N,length(zg))

for (j in 1:length(zg))
{
  t_vec <- c()
  for (i in 1:N)
  {
...
```{r}
tib <- tibble(x=zg,y=apply(P>0.05,2,mean))
